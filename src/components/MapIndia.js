...
class MapIndia extends Component {
    ...
    render() {
        return (
            <div>
                <div id="IndiaMap"></div>
                <div className="mapTooltipLeft" id="mapTooltip">
                    <div className="mapTooltipHeader">
                    <div align="center"><span id="mapTooltipHeader"></span></div>
                    </div>
                    <div className="mapTooltipValue">
                    <div align="center">
                        <span  id="mapTooltipValue"></span>
                    </div>                      
                    </div>
                </div>
            </div>
    ...
    componentDidMount(){
        ...
        let offsetWidth = document.getElementById('IndiaMap').offsetWidth - 500
        let IndiaMapSvg = d3.select('#IndiaMap').append('svg')
            .attr('height','490px')
            .attr('width',offsetWidth-10+'px')
            //.style('background-color','#f4f4f4')
            .attr("transform","translate("+(5)+","+(5)+")")
            .style('border-radius','8px')
            .style('border','0.5px solid black');
        let pieChart = d3.select('#IndiaMap').append('svg')
            .attr('height','490px')
            .attr('width','490px')
            //.style('background-color','#f4f4f4')
            .attr("transform","translate("+(5)+","+(5)+")")
        ...
        let g = IndiaMapSvg.append('g')
            .attr("transform","translate("+(80)+","+(10)+")")
        console.log(offsetWidth)
        this.drawChart(IndiaMapSvg,g,offsetWidth-10,490)
        this.drawPieChart(data,pieChart,490,490)
        this.addTitlePieChart(pieChart)
        this.addlegendsPieChart(data,pieChart)
    }
    drawChart(IndiaMapSvg,g,width,height){
        let projection = d3.geoMercator();

        let path = d3.geoPath()
            .projection(projection)
            .pointRadius(2);
        let boundary = this.centerZoom(this.state.india_coordinates,projection,path,width,height)
        let subunits = this.drawSubUnits(this.state.india_coordinates,g,path);
        this.colorSubUnits(this.state.india_coordinates,subunits,path)
        this.addlegends(IndiaMapSvg)
        this.addTitle(IndiaMapSvg)
        //console.log(subunits)
    }
    addTitle(IndiaMapSvg){
        let svgTitle = IndiaMapSvg.append('g')
    ...
    centerZoom(data,projection,path,width,height){
        let o = topojson.mesh(data, data.objects.polygons, function(a, b) { return a === b; });
    ...
    drawSubUnits(data,IndiaMapSvg,path){
        let subunits = IndiaMapSvg.selectAll(".subunit")
            .data(topojson.feature(data, data.objects.polygons).features)
            .enter().append("path")
                .attr("className", "subunit")
                .attr("d", path)
                .style("stroke", "#fff")
    ...
    //colorSubUnits method will add color to each state and will also add tooltip 
    colorSubUnits(data,subunits,path){
        let corona_details = [
            {
        ...
        let min = 100000,
            max = -100000
        let tool = d3.select('#mapTooltip');
        let html = ""
        corona_details.forEach( e1 =>{
            if(e1.positiveCase > max){
                max = e1.positiveCase
            } else if(e1.positiveCase < min && e1.positiveCase > 0){
                min = e1.positiveCase
            }
            topojson.feature(data, data.objects.polygons).features.forEach( e2 =>{
                if(e1.st_nm === e2.properties.st_nm){
                    e2.properties.positiveCase = e1.positiveCase
                }
            })
        })
        //console.log(min,max)
        let color = d3.scaleOrdinal()
            .domain([min,max/4,max/2,(3*max)/4,max])
            .range(['#ffb5c5','#db889a','#ff6e8d','#db4868','#992840'])
        let singleUnit = subunits
            ...
            .on('mousemove',function(d){
                ...
                const y = coords[1] + 125;
                tool.style('visibility', 'visible');
                tool.attr('transform', `translate(${x},${y})`)
                d3.select('#mapTooltipHeader')
                    .text(d.properties.st_nm)
                d3.select('#mapTooltipValue')
                    .text("No of Positive Cases : "+d.properties.positiveCase)
                tool.style("left", d3.event.pageX + 25 + "px")
                tool.style("top", d3.event.pageY - 40 + "px")
    ...
    addlegends(IndiaMapSvg){
        let legends = IndiaMapSvg.append('g')
            .attr("transform","translate("+(10)+","+(400)+")")

        //legends.append(text).text('Number Of Positive Cases :')
        let w = 200, h = 15;
        let legend = legends.append("defs")
        ...
        legend.append("stop")
            .attr("offset", "0%")
            .attr("stop-color", '#ffb5c5')
            .attr("stop-opacity", 1);
      
        legend.append("stop")
            .attr("offset", "25%")
            .attr("stop-color",'#db889a')
            .attr("stop-opacity", 1);
      
        legend.append("stop")
            .attr("offset", "50%")
            .attr("stop-color",'#ff6e8d')
            .attr("stop-opacity", 1);
        legend.append("stop")
            .attr("offset", "75%")
            .attr("stop-color",'#db4868')
            .attr("stop-opacity", 1);
        
        legend.append("stop")
            .attr("offset", "100%")
            .attr("stop-color",'#992840')
            .attr("stop-opacity", 1);
        ...
        let yAxis = d3.axisBottom()
            .scale(y)
            .tickValues(y.ticks(1).concat(y.domain()))
    ...
    drawPieChart(data,pieChart,width,height){
        let corona_details = [
            {
                "st_nm" : "Andhra Pradesh",
                "positiveCase" : 5
    ...
    addTitlePieChart(pieChart){
        let svgTitle = pieChart.append('g')
            .attr("transform","translate("+(10)+","+(30)+")")
        svgTitle.append('text')
    ...
    addlegendsPieChart(data,pieChart){
        let color = d3.scaleOrdinal()
            .range(["#2882d1", "#ff6666", "#ff99bb"]);
        ...
        data.forEach( (element,index) => {
            ...
            svgLegend.append('circle')
                ...
                .attr('cy', 20 + index*30)
                .attr('r',10)
                .style('fill',color(element.type))
            svgLegend.append('text')
                .text(element.type)
                .attr('x',50)
                .attr('y',23.5+30*index)
